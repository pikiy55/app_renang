...
@section('title', 'Tambah KU - ' . $event->nama_event)
@section('header_title', 'Tambah Kelompok Umur')
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden max-w-3xl">
    <div class="p-8">
        <form action="{{ route('admin.events.ku.store', $event) }}" method="POST">
            @csrf
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="nama_ku" id="nama_ku" value="{{ old('nama_ku') }}" required
                        ...
                        placeholder="Contoh: KU I (6-7 tahun)">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="usia_min" id="usia_min" value="{{ old('usia_min') }}" min="0"
                            class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-3 px-4 bg-slate-50 hover:bg-white transition-colors"
                    ...
                    <div>
                        ...
                        <input type="number" name="usia_max" id="usia_max" value="{{ old('usia_max') }}" min="0"
                            class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-3 px-4 bg-slate-50 hover:bg-white transition-colors"
            ...
            <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end space-x-4">
                ...
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white hover:bg-indigo-700 transition-all transform hover:-translate-y-0.5">Simpan KU</button>
            </div>
        </form>
    </div>
</div>
